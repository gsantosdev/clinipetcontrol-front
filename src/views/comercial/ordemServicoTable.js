    ...
    const rows = ordens.slice(pagesVisited, pagesVisited + ordensPerPage).map((ordem, index) => {
        return (
            <tr style={{ backgroundColor: index % 2 ? "rgb(250,250,250)" : "rgb(241,241,241" }} key={index}>
                ...
                {ordem.statusOrdem == "EFETIVADO" ? <td style={{ color: "rgb(78 173 47)" }}>{ordem.statusOrdem}</td> : false}
                ...
                <td>
                    {ordem.statusOrdem === "PENDENTE" ?
                        <>
                            <button type="button" className="btn" style={{ color: "white", backgroundColor: 'blue' }}>EXECUTAR</button>
                            <button type="button" className="btn btn-danger">CANCELAR</button>
                        </> : false}
                    {ordem.statusOrdem === "EM_EXECUCAO" ?
                        <>
                            <button type="button" className="btn btn-success">CONCLUIR</button>
                            <button type="button" className="btn btn-danger">CANCELAR</button>
                        </>
                        : false}
